...
class Paginator {
  ...
  async paginate() {
    ...
    switch (this._type) {
      case PAGINATION_TYPE.SCROLL:
        return this.paginateScroll();

      case PAGINATION_TYPE.PAGE:
        return this.paginatePage();

      case PAGINATION_TYPE.SHOW_MORE:
        return this.paginateShowMore();
      ...
      default:
        ...
        return this.paginateCustom(customPagination.paginate, customPagination.checkPaginate, this._customOptions);
    }
  }
   ...
   * @private
   */
  async paginateCustom(paginateFn, checkPaginateFn, options) {
    try {
   ...
   * @private
   */
  async paginateScroll() {
    try {
      const previousHeight = await this._env.evaluateJs(/* @covignore */ function () {
        return document.body.scrollHeight;
      });
      await this._actions.scroll(this._scrollInterval);
      await this.hasPageScrolled(previousHeight);
      debug('page %s opened', this._currentPage);
      return {
        value: this._currentPage,
        done: false,
   ...
   * @private
   */
  async getRealNextPageIndex() {
    const pages = await this._env.evaluateJs(this._scope, /* @covignore */ function (selector) {
  ...
  async paginatePage() {
    try {
      const nextPageIndex = await this.getRealNextPageIndex();
      const selector = `${this._scope}:eq(${nextPageIndex})`;
      const html = await this._env.evaluateJs(this._pageScope, this.getPaginatePageHtml);
      ...
      switch (this._strategy) {
        ...
        case PAGINATION_STRATEGY.AJAX:
          await this.hasPageDataChanged(html);
          break;
        case null:
          await any([
            ...
            this.hasPageDataChanged(html).then(() => {
   ...
   * @private
   */
  getPaginatePageHtml/* @covignore */(selector) {
   ...
   * @private
   */
  async hasPageScrolled(previousPageHeight) {
   ...
   * @private
   */
  async hasPageDataChanged(previousPageHtml) {
    return this._actions.wait(this.getPaginatePageHtml, (html) => {
